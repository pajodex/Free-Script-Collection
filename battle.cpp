		...
		case PA_SHIELDCHAIN:
			...
			if (sd) {
				...
				if (index >= 0 && sd->inventory_data[index] && sd->inventory_data[index]->type == IT_ARMOR) {
					ATK_ADD(wd.damage, wd.damage2, sd->inventory_data[index]->weight / 1);
#ifdef RENEWAL
					ATK_ADD(wd.weaponAtk, wd.weaponAtk2, sd->inventory_data[index]->weight / 1);
#endif
				}
			} else
...
static struct Damage battle_calc_weapon_attack(struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv, int wflag)
{
	...
	if (skill_id == MO_EXTREMITYFIST) 
		wd.damage = cap_value(wd.damage, INT_MIN, 400000);
...
#define MATK_ADD(a) { ad.damage += a; }
		...
		if(!flag.imdef){
			...
			ad.damage = ad.damage * (5000 + mdef) / ( 1000 + mdef * 10) - mdef2; // DXRO
#else

...
unsigned short status_base_atk(const struct block_list *bl, const struct status_data *status)
{
	...
	**/
	...
	if (bl->type == BL_PC)
#ifdef RENEWAL
		str = (dstr*10 + dex*10/5 + status->luk*10/3 + ((TBL_PC*)bl)->status.base_level*10/4)/5; // DXRO
	...
	**/
	...
	if(flag&SCB_MDEF2) {
		if (status->int_ == b_status->int_ && status->vit == b_status->vit
			...
			+ (int)(((float)status->vit / 6 - (float)b_status->vit / 6))
#else

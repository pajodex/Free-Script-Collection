...
void initChangeTables(void)
{
	...
	set_sc(SM_MAGNUM, SC_FIREWEAPON	, SI_FIREWEAPON, SCB_NONE);
	set_sc( SM_ENDURE		, SC_ENDURE		, SI_ENDURE		, SCB_MDEF|SCB_DSPD );
	set_sc( BS_UNFAIRLYTRICK, SC_ENDURE, SI_ENDURE, SCB_MDEF | SCB_DSPD);
	...
	add_sc( CR_SHIELDBOOMERANG	, SC_STUN);
	add_sc( KN_CHARGEATK		, SC_STUN);
	...
	set_sc(MS_MAGNUM, SC_FIREWEAPON			, SI_FIREWEAPON, SCB_ATK_ELE);
...
unsigned short status_base_atk(const struct block_list *bl, const struct status_data *status)
{
	...
	**/
	...
	if (bl->type == BL_PC)
#ifdef RENEWAL
		str = (dstr*10 + dex*10/7 + status->luk*10/3 + ((TBL_PC*)bl)->status.base_level*10/4)/5; // /10 original ... dex*10/5
...
unsigned int status_weapon_atk(struct weapon_atk wa, struct map_session_data *sd)
{
	...
	return wa.atk + wa.atk2 + (int)(wa.atk * (str/200) + weapon_atk_bonus); // 200 -> 100     DXRO
}
#endif
...
unsigned short status_base_matk(struct block_list *bl, const struct status_data* status, int level)
{
	switch (bl->type) {
		...
		default:
			//return status->int_ + (status->int_ / 2) + (status->dex / 5) + (status->luk / 3) + (level / 4);
			return status->int_ + (status->int_ / 1) + (status->dex / 3) + (status->luk / 3) + (level / 4);
	}
}
#endif
...
void status_calc_misc(struct block_list *bl, struct status_data *status, int level)
{
	...
	if (bl->type == BL_HOM) {
		...
		// Atk
		stat = (status_get_homstr(bl) + status_get_homdex(bl)) / 5; // 5
		...
		// Flee
		stat = status->flee;
		stat += level + status->agi + (bl->type == BL_MER ? 0 : bl->type == BL_PC ? status->luk / 5 : 0); //base level + ( every 1 agi = +1 flee ) + (every 5 luk = +1 flee) + 100
		status->flee = cap_value(stat, 1, SHRT_MAX);
		// Def2
		if (bl->type == BL_MER)
			...
			stat += (int)(((float)level + status->vit) / 2 ); //removed agi DXRO
		}
		status->def2 = cap_value(stat, 0, SHRT_MAX);
		// Mdef2
		if (bl->type == BL_MER)
			...
			stat += (int)(bl->type == BL_PC ? (status->int_ + ((float)level / 4) + ((float)(status->vit) / 5)) : ((float)(status->int_ + level) / 4)); // Remove dex from calc DXRO
		}
		status->mdef2 = cap_value(stat, 0, SHRT_MAX);
	}
	...
	**/
	...
	if(flag&SCB_DEF2) {
		if (status->vit == b_status->vit
			...
			+ (int)(((float)status->vit / 3 - (float)b_status->vit / 3)) //+ ((float)status->agi/5 - (float)b_status->agi/5) )
#else
...
#ifdef RENEWAL
			+ (int)( /*((float)status->dex/5 - (float)b_status->dex/5) +  ((float)status->vit/5 - (float)b_status->vit/5) )*/ ((float)status->vit/6 - (float)b_status->vit/6) )
#else
	...
	if(flag&SCB_CRI && b_status->cri) {
		...
		if( bl->type == BL_PC && ((TBL_PC*)bl)->status.weapon == W_KATAR )
			status->cri <<= 1;
		if (bl->type == BL_PC && ((TBL_PC*)bl)->status.weapon == W_BOOK)
			status->cri <<= 1;
...
#else
			b_status.rhw.atk2 != status->rhw.atk2 || b_status.lhw.atk2 != status->lhw.atk2 //rene
#endif
			)
		...
		case SC_WEAPONPERFECTION:
			{
				...
				if (s_sd && pc_checkskill(s_sd, BS_HILTBINDING) > 0)
					tick += tick / 30; //If caster has Hilt Binding, duration increases by 10%
			}
			break;
